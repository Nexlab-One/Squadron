...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.squadronApiUrl)) {
    const squadronApiUrlFromInput = nonEmptyTrimmedString(input.squadronApiUrl);
    if (squadronApiUrlFromInput) merged.squadronApiUrl = squadronApiUrlFromInput;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    squadronApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.squadronApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const gatewayVariantRaw = nonEmptyTrimmedString(defaults.gatewayVariant);
  const isMoltis = gatewayVariantRaw?.toLowerCase() === "moltis";
  if (isMoltis) {
    normalized.gatewayVariant = "moltis";
  }
  ...
  if (configuredDevicePrivateKeyPem) {
  ...
  } else if (!disableDeviceAuth && !isMoltis) {
    try {
    ...
    diagnostics.push({
      code: "openclaw_gateway_moltis_no_device_auth",
      level: "info",
      message: "Moltis gateway: token auth only; device key not required."
    });
  }
  ...
  const rawSquadronApiUrl =
    typeof defaults.squadronApiUrl === "string"
      ? defaults.squadronApiUrl.trim()
      : "";
  if (rawSquadronApiUrl) {
    try {
      const parsedSquadronApiUrl = new URL(rawSquadronApiUrl);
      if (
        parsedSquadronApiUrl.protocol !== "http:" &&
        parsedSquadronApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_squadron_api_url_protocol",
          level: "warn",
          message: `squadronApiUrl must use http:// or https:// (got ${parsedSquadronApiUrl.protocol}).`
        });
      } else {
        normalized.squadronApiUrl = parsedSquadronApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_squadron_api_url_configured",
          level: "info",
          message: `squadronApiUrl set to ${parsedSquadronApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_squadron_api_url_invalid",
        level: "warn",
        message: `Invalid squadronApiUrl: ${rawSquadronApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm squadron allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/paperclip-claimed-api-key.json and load SQUADRON_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: squadronApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Squadron hostname is reachable and allowed via `pnpm squadron allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Squadron API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          squadronApiUrl: "http://host.docker.internal:3100",
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "squadronApiUrl": "https://squadron-hostname-your-agent-can-reach:3100",
    ...
    And set the SQUADRON_API_KEY and SQUADRON_API_URL in your environment variables as specified here:
    ...
    Then set SQUADRON_API_KEY and SQUADRON_API_URL from the saved token field for every heartbeat run.
    ...
    Be sure to prepend your SQUADRON_API_URL to the top of your skill and note the path to your SQUADRON_API_URL
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Squadron base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.squadronApiUrl when submitting your join request
  ...
  appendBlock(`
    ...
    ## Using Moltis gateway
    To join with a Moltis gateway (client-first, protocol 4): use adapterType "openclaw_gateway"
    and set agentDefaultsPayload.gatewayVariant to "moltis". Set url to your Moltis WebSocket
    (e.g. ws://host:port/ws/chat). Same headers (x-openclaw-token or Authorization) and
    squadronApiUrl. Device key is not required for Moltis. Install Moltis, start the gateway
    (e.g. moltis serve or from moltis-main docs), then submit the join request as above with
    gatewayVariant: "moltis" in agentDefaultsPayload. Approve and claim as for OpenClaw.
    See doc/MOLTIS_ONBOARDING.md for operator steps.
...
export function accessRoutes(
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const moltisVariant = req.body.gatewayVariant === "moltis";
      const { token, created, normalizedAgentMessage } =
        await createCompanyInviteForCompany({
          req,
          companyId,
          allowedJoinTypes: "agent",
          defaultsPayload: moltisVariant ? { gatewayVariant: "moltis" } : null,
          agentMessage: req.body.agentMessage ?? null
        });
      ...
      await logActivity(db, {
        ...
        details: {
          ...
          expiresAt: created.expiresAt.toISOString(),
          hasAgentMessage: Boolean(normalizedAgentMessage),
          gatewayVariant: moltisVariant ? "moltis" : undefined
        }
      });
      ...
      res.status(201).json({
        ...
        onboardingTextUrl: inviteSummary.onboardingTextUrl,
        inviteMessage: inviteSummary.inviteMessage,
        ...(moltisVariant ? { gatewayVariant: "moltis" as const } : {})
      });
    }
  );
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              squadronApiUrl: req.body.squadronApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.squadronApiUrl &&
          !persistedDefaults.squadronApiUrl
        ) {
          missingPersistedFields.push("squadronApiUrl");
        }
