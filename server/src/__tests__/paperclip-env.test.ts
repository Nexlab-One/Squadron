...
const ORIGINAL_SQUADRON_API_URL = process.env.SQUADRON_API_URL;
...
afterEach(() => {
  if (ORIGINAL_SQUADRON_API_URL === undefined) delete process.env.SQUADRON_API_URL;
  else process.env.SQUADRON_API_URL = ORIGINAL_SQUADRON_API_URL;
...
describe("buildPaperclipEnv", () => {
  it("prefers an explicit SQUADRON_API_URL", () => {
    process.env.SQUADRON_API_URL = "http://localhost:4100";
    process.env.PAPERCLIP_LISTEN_HOST = "127.0.0.1";
    process.env.PAPERCLIP_LISTEN_PORT = "3101";

    const env = buildPaperclipEnv({ id: "agent-1", companyId: "company-1" });

    expect(env.SQUADRON_API_URL).toBe("http://localhost:4100");
  });

  it("uses runtime listen host/port when explicit URL is not set", () => {
    delete process.env.SQUADRON_API_URL;
    ...
    expect(env.SQUADRON_API_URL).toBe("http://localhost:3101");
  });
  ...
  it("formats IPv6 hosts safely in fallback URL generation", () => {
    ...
    expect(env.SQUADRON_API_URL).toBe("http://[::1]:3101");
  });
});
