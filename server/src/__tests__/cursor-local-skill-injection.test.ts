...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Squadron skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("squadron-cursor-skills-src-");
    const skillsHome = await makeTempDir("squadron-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "squadron");
    await createSkillDir(skillsDir, "squadron-create-agent");
    ...
    const injectedA = path.join(skillsHome, "squadron");
    const injectedB = path.join(skillsHome, "squadron-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "squadron")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "squadron-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "squadron"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "squadron-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("squadron-cursor-preserve-src-");
    const skillsHome = await makeTempDir("squadron-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "squadron");
    ...
    expect((await fs.lstat(path.join(skillsHome, "squadron-create-agent"))).isSymbolicLink()).toBe(true);
  });
