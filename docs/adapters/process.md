...
- Running a Python script that calls the Squadron API
...
1. Squadron spawns the configured command as a child process
2. Standard Squadron environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Squadron API and perform work.
