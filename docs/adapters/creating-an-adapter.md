...
Build a custom adapter to connect Squadron to any agent runtime.
...
`format-event.ts` — pretty-prints stdout for `squadron run --watch` using `picocolors`.
...
Make Squadron skills discoverable to your agent runtime without writing to the agent's working directory:
