...
Squadron is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── paperclip/               # Core Squadron skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Squadron env vars and a prompt
4. **Agent work** — The agent calls Squadron's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Squadron and agent runtimes. Each adapter is a package with three modules:
...
- **Control plane, not execution plane** — Squadron orchestrates agents; it doesn't run them
