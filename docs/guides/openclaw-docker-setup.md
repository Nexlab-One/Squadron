...
How to get OpenClaw running in a Docker container for local development and testing the Squadron OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Squadron includes an end-to-end join smoke harness:
...
- probes and prints a Squadron host URL that is reachable from inside OpenClaw Docker
...
If your Squadron deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Squadron server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Squadron URL (typically `http://host.docker.internal:3100`).
- If Squadron rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm squadron allowed-hostname host.docker.internal
```

Then restart Squadron and rerun the smoke script.
