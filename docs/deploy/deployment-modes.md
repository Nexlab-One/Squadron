...
Squadron supports two runtime modes with different security profiles.
...
```sh
pnpm squadron allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Squadron emits a one-time claim URL at startup:
...
```sh
PAPERCLIP_DEPLOYMENT_MODE=authenticated pnpm squadron run
```
