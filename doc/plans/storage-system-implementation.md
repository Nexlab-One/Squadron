...
- `squadron onboard` writes a valid `storage` config block by default.
- `squadron configure --section storage` can switch between local and s3 modes.
...
- `squadron doctor` reports actionable storage status.
