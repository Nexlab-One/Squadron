...
- local mode that still feels instant (`npx squadron run` and go)
...
2. CLI command `pnpm squadron auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm squadron onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
...
- expose key instance settings in API + CLI (`squadron configure` / `squadron onboard`)
...
7. `pnpm squadron onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
